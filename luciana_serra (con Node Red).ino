...
float temp_min = 26, temp_max = 32;
float hum_min = 0.40, hum_max = 40;
...
void loop()
{
  while (Serial.available() > 0)
  {
    ...
    switch (inbyte)
    {
      ...
      case 'a':
        hum_min = Serial.parseFloat();
        break;

      case 'b':
        hum_max = Serial.parseFloat();
        break;

      case 'c':
        temp_min = Serial.parseFloat();
        break;

      case'd':
        temp_max = Serial.parseFloat();
        break;

      default:

        /*QUA PARTE IL PROGRAMMA VERO E PROPRIO IN DEFAULT
          Lo switch case serve per leggere i dati da Node-Red
        */
        lcd.setCursor(0, 0);
       ...
       //ACCENSIONE/SPENGIMENTO POMPA DELL'ACQUA
        ...
        if (digitalRead(pin_water) == HIGH) stato_acqua = true;
        else stato_acqua = false;
        ...
        Serial.print(",\"acqua\":");
        Serial.print(stato_acqua);
        Serial.print(",\"riscaldamento\":");
        Serial.print(rele_temp);
        Serial.print("}\n");
